...
export const TextNode = ({ id, data }) => {
  ...
  const handleBlur = () => { if (data?.text !== currText) updateNodeField(id, 'text', currText); };

  useEffect(() => {
    if (data?.text !== undefined && data.text !== currText) setCurrText(data.text);
  ...
  const varHandles = variables.map((variable, index) => {
    const topPct = variables.length === 1 ? 50 : 20 + (60 / (variables.length - 1)) * index;
    return {
      type: 'target',
